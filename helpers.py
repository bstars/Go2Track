...
class Pi(nn.Module):
    ...
    For SAC, output a tanh-transformed gaussian
    """
    def __init__(self, state_dim, action_dim, hidden_dim, tanh_transformed=False, init_log_std=0.):
        super().__init__()
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.tanh_transformed = tanh_transformed
        self.net = construct_mlp([state_dim] + hidden_dim + [action_dim])
    ...
    def forward(self, states):
        ...
        if not self.tanh_transformed:
            ...
            # y = T(x)
		    # P(y) = P(x) / |T'(x)|
            # did not use from torch.distributions.transforms import TanhTransform for explicit numerical stability
            actions_transformed = torch.tanh(actions)
            log_probs = normal.log_prob(actions)
            log_probs_transformed = log_probs - torch.log(1 - actions_transformed**2 + 1e-6)
            log_probs_transformed = log_probs_transformed.sum(-1)

            return normal, actions_transformed, log_probs_transformed
